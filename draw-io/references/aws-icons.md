...
- 使用 `mxgraph.aws4.*` (aws3 已过时不推荐使用)
- 服务名称使用 snake_case 指定
...
### 3.6. Security, Identity & Compliance (安全)

| 服务 | resIcon |
|----------|---------|
| AWS IAM | mxgraph.aws4.identity_and_access_management |
| IAM (短缩) | mxgraph.aws4.iam |
...
### 3.8. Machine Learning & AI (机器学习・AI)
...
| Machine Learning (通用) | mxgraph.aws4.machine_learning |

### 3.9. Application Integration (应用集成)
...
### 3.10. Management & Governance (管理・治理)
...
### 3.14. Business Applications (业务应用)
...
### 3.15. End User Computing (终端用户计算)
...
### 3.22. Quantum Technologies (量子技术)
...
### 3.25. General/Generic Icons (通用图标)
...
使用 [scripts/find_aws_icon.py](../scripts/find_aws_icon.py) 可以高效搜索。
